...
/**
 * FILTRO SUPREMO MEJORADO (Optimizado a modelo 8B para ahorro de cuota diaria)
 */
async function filtrarArticulosRelevantes(pregunta, articulosCandidatos) {
  ...
  try {
    const chatCompletion = await groq.chat.completions.create({
      ...
      model: 'llama-3.1-8b-instant', // <--- Balance de carga eficiente para JSON estructurado
      temperature: 0.1,
...
app.post('/api/consultar', async (req, res) => {
  ...
  try {
    // 1. Clasificación Procesal (Migrada a 8B para conservar ancho de banda)
    ...
    const resClasificacion = await groq.chat.completions.create({
      ...
      model: 'llama-3.1-8b-instant', // <--- Rápido y económico para metadatos
      temperature: 0.1,
      response_format: { type: "json_object" }
    });
    ...
    *   **Cobro de Títulos Valores (Letra de Cambio):** La Letra de Cambio es un título valor con aparejada ejecución conforme al Código de Comercio. Su falta de pago faculta al tenedor legítimo a demandar mediante el **Procedimiento de Intimación** (Art. 640 CPC) solicitando simultáneamente el embargo preventivo de bienes, o a ejercer la acción cambiaria en vía ejecutiva mercantil. No se debe encuadrar el cobro de una letra de cambio vencida dentro de las fases ordinarias declarativas de un juicio civil común (demanda, contestación ordinaria, lapso probatorio general) a menos que medie oposición fundada del intimado.
    ...
    // 4. GENERACIÓN CON TOLERANCIA A FALLAS (Redundancia activa Llama-3.3-70b -> Llama-3.1-8b)
    let responseFinal;
    try {
      console.log(`${timestamp} 🧠 Intentando generación con Llama-3.3-70b-versatile...`);
      ...
      // Si interceptamos un error 429 de Rate Limit, ejecutamos el Fallback de inmediato
      if (innerError.status === 429) {
        console.warn(`${timestamp} ⚠️ Rate Limit detectado en 70B. Conmutando de emergencia a Llama-3.1-8b-instant.`);
        responseFinal = await groq.chat.completions.create({
          messages: [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: promptFinal }
          ],
          model: 'llama-3.1-8b-instant', // <--- Modelo salvavidas con límites holgados
          temperature: 0.3
        });
      } else {
        throw innerError; // Si es un error distinto, se arroja al catch principal
      }
    }
...
app.listen(PORT, () => {
  console.log(`🚀 LexnaVe Backend activo (Infraestructura Híbrida Groq) en el puerto ${PORT}`);
});
